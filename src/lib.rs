...
#[no_mangle] pub extern "C" fn zos_plugin_name() -> *mut c_char { to_c("orbits") }
#[no_mangle] pub extern "C" fn zos_plugin_version() -> *mut c_char { to_c("0.2.0") }
#[no_mangle] pub extern "C" fn zos_plugin_commands() -> *mut c_char { to_c("simulate,themes,project-8d") }
...
#[no_mangle] pub extern "C" fn zos_plugin_execute(cmd: *const c_char, arg: *const c_char) -> *mut c_char {
    ...
    let result = json!({"plugin": "orbits", "command": cmd, "arg": arg, "status": "ok"});
    let shard = DA51Shard::from_result("orbits", cmd, &result);
    to_c(&serde_json::to_string(&json!({"result": result, "shard": shard})).unwrap())
}

#[no_mangle] pub extern "C" fn zos_plugin_render() -> *mut c_char {
    let gui = vec![
        GuiComponent::Heading { level: 2, text: "🛸 Orbits".into() },
